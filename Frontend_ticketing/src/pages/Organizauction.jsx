    ...
    switch (step) {
      case 1:
        return (
          ...
          >
            <h2 className="text-2xl font-semibold mb-4 text-black">Step 1: Select a Ticket</h2>
            {isLoading ? (
              <LoadingSpinner />
            ) : (
              <ul className="space-y-4">
                {userTickets.map((ticket, index) => (
                  <motion.li
                    ...
                    whileTap={{ scale: 0.95 }}
                  >
                    <p className="font-semibold text-black">{ticket.event.name}</p>
                    <p className="text-sm text-black">Date: {new Date(ticket.event.date).toLocaleDateString()}</p>
                    <p className="text-sm text-blue-500">Current Price: {formatPrice(ticket.price)}</p>
                  </motion.li>
                ))}
              </ul>
            )}
      ...
      case 2:
        return (
          <motion.div
            ...
            transition={pageTransition}
          >
            <h2 className="text-2xl font-semibold mb-4 text-black">Step 2: Set Auction Details</h2>
      ...
      case 3:
        return (
          <motion.div
            ...
            transition={pageTransition}
          >
            <h2 className="text-2xl font-semibold mb-4 text-black">Step 3: Preview and Create Auction</h2>
            <AuctionPreview
...
const AnimatedInput = ({ id, label, value, onChange, type, tooltipContent }) => {
...
const AuctionPreview = ({ selectedTicket, startingBid, auctionEnd, formatPrice }) => (
  <motion.div
  ...
  >
    <h3 className="text-xl font-semibold mb-2 text-black">{selectedTicket.event.name}</h3>
    <p className="text-black mb-2">
      <span className="font-semibold">Date:</span> {new Date(selectedTicket.event.date).toLocaleDateString()}
    </p>
    <p className="text-blue-500 mb-2">
      <span className="font-semibold">Starting Bid:</span> {formatPrice(startingBid)}
    </p>
    <p className="text-red-400 mb-4">
      <span className="font-semibold">Auction Ends:</span> {new Date(auctionEnd).toLocaleString()}
    </p>

    <div className="bg-amber-100 p-4 rounded-md">
      <h4 className="text-lg font-semibold mb-2 text-amber-800">Time Remaining:</h4>
        ...
        <>
          {Object.entries(timeLeft).map(([unit, value]) => (
            <motion.div
              ...
              transition={{ duration: 0.5 }}
            >
              <span className="text-2xl font-bold text-amber-800">{addLeadingZero(value)}</span>
